...
import { isMouseEvent, noop, throttle, verticalPositionInRect } from '../../utils';
...
export class ParagraphFrontButton extends BaseFloat {
	public pluginName = 'frontButton';
	...
	override listen() {
		...
		const mousemoveHandler = throttle((event: MouseEvent) => {
			...
			if (outMostElement) {
				this.show(outMostElement[BLOCK_DOM_PROPERTY] as unknown as ReferenceElement);
				this.render();
			}
			else {
				this.hide();
	...
	override show(reference: ReferenceElement, _cb = noop) {
		const block = reference as unknown as Parent;
