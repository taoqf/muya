...
import type BaseFloat from './ui/baseFloat';
import type { IBaseOptions } from './ui/types';
import {
	CLASS_NAMES,
...
export class Muya {
	...
	static use<T extends typeof BaseFloat, O extends IBaseOptions>(plugin: T, options: O = {} as O) {
		this.plugins.push({
			plugin,
			options,
		});
